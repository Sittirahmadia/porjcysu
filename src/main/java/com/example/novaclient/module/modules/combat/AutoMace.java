...
public class AutoMace extends Module {
    ...
    private static final RegistryKey<Enchantment> DENSITY_KEY = RegistryKey.of(RegistryKeys.ENCHANTMENT, Identifier.of("minecraft", "density"));
    ...
    public AutoMace() {
        super("Auto Mace", "Automatically attacks with mace", Category.COMBAT, GLFW.GLFW_KEY_UNKNOWN);
        addSettings(minFallDistance, attackDelay, densityThreshold, targetPlayers, targetMobs,
                stunSlam, onlyAxe, autoSwitch, stayOnMace);
    }
    ...
    private void updateFall() {
        ...
        if (mc.player.getVelocity().y > 0.1 && maceHit) {
            maceHit = false;
            fallStartY = currentY;
        }
    ...
    private void attack() {
        ...
        if (target instanceof PlayerEntity player
                && NovaClient.getInstance().getFriendManager().isFriend(player.getName().getString()))
            return;
    ...
    private int findDensityMaceSlot() {
        for (int i = 0; i < 9; i++) {
            ...
            if (stack.getItem() == Items.MACE && EnchantmentUtil.hasEnchantment(stack, mc.world, DENSITY_KEY)) return i;
        }
        return -1;
    }
    ...
    private int findBreachMaceSlot() {
        for (int i = 0; i < 9; i++) {
            ItemStack stack = mc.player.getInventory().getStack(i);
            if (stack.getItem() == Items.MACE && EnchantmentUtil.hasEnchantment(stack, mc.world, BREACH_KEY)) return i;
        }
        return -1;
    }
    ...
    public void onDisable() {
        if (savedSlot != -1) switchToSlot(savedSlot);
        savedSlot = -1;
        fallStartY = -1;
        isFalling = false;
        slamExecuted = false;
